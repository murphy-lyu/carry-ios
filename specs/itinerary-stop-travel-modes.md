...
**公交（2026-06-15 追加）**：评估后纳入公交（含地铁）。与骑行相反——公交四家 deep-link 文档上**都支持**（含 Apple，Apple 有 transit 只是没 cycling），故大概率不过滤任何一家。不显时长这条不变（外部地图自带班次/路线）。真机实测确认各家可正常调起后定稿过滤矩阵。

## 做什么

详情底部路程模块：在 Get Directions 之上加一个**交通方式选择器**（**驾车（默认）/ 公交 / 步行 / 骑行**）。选中某方式 → Get Directions **调起外部地图时用该方式**导航。直线距离「到下一站 N 公里」保留（离线、瞬时）。

```
┌─────────────────────────────────────────────┐
│  [🚗 驾车] [🚌 公交] [🚶 步行] [🚴 骑行]      │  ← 选择器，默认驾车
│  ─────────────────────────────────────────   │
...
| App | 驾车 | 公交 | 步行 | 骑行 | URL 参数 |
|---|---|---|---|---|---|
| Apple 地图 | ✅ | ✅ | ✅ | **❌** | `MKLaunchOptionsDirectionsModeKey`：Driving/Transit/Walking（**无骑行**）|
| 高德 | ✅ | ✅ | ✅ | ✅ | `iosamap://path?...&t=` → 驾车 0 / 公交 1 / 步行 2 / 骑行 3 |
| Google 地图 | ✅ | ✅ | ✅ | ✅ | `comgooglemaps://?...&directionsmode=` → driving/transit/walking/bicycling |
| 百度 | ✅ | ✅ | ✅ | ✅ | `baidumap://map/direction?...&mode=` → driving/transit/walking/riding |

**关键规则**：**选「骑行」时，Get Directions 的 App List 隐藏 Apple 地图**（它不支持），其余支持骑行的（高德/百度/Google）照常。驾车/公交/步行时四家都在（公交待真机实测，若某家不灵再在 `supports` 精修）。
...
- **不在 App 内显时长**（Apple 无骑行时长 + 不接 API；公交亦不显时长，外部地图自带）。直线距离仍显。
- **不接**高德/百度路由 Web API。
