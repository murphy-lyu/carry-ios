...
> 5. 停靠点类型 `StopCategory` = **7 类**：`sightseeing`（景点）/ `food`（餐饮）/ `lodging`（住宿）/ `transport`（交通节点：高铁/地铁/公交）/ `flight`（航班：机场/飞机，2026-06-13 新增）/ `activity`（活动）/ `other`（其他）。
...
- `StopCategory` 枚举（rawValue 存库，UI 用本地化 key）= **7 类**：`sightseeing`（景点）/ `food`（餐饮）/ `lodging`（住宿）/ `transport`（交通节点：高铁/地铁/公交）/ `flight`（航班：机场/飞机）/ `activity`（活动）/ `other`（其他）。新增 case 是加值变更、向后兼容（旧数据用旧 rawValue，未知值兜底为 `.other`），无需迁移。
...
- `itinerary.category.sightseeing` / `.food` / `.lodging` / `.transport` / `.flight` / `.activity` / `.other`
