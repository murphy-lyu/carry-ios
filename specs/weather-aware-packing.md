...
1. 阈值（降水概率 / 高低温 / 温差）真机校准，避免过敏感或漏报。
2. 天气打包 nudge 呈现：天气卡下行动 chip（首选）vs Surprise 卡（次选）——落地时按视觉密度二选一。
3. 「天气提醒」默认开/关：建议**开**（例外驱动量小）；若顾虑通知许可，可首单引导。
4. `ClimateInference` 是否随真实天气进入后"退场"，还是始终作为远期底（建议后者：分层共存）。
