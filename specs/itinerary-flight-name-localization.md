...
- **航司**：`FlightNameCache`（`Carry/Models/FlightNameCache.swift`，`@MainActor`，同步、懒加载 memoise `airlines.json` 225K）。`displayCarrier(for:)` = 航班则从航班号 `split` 出 IATA → 本地化 `displayName`；非航班/未识别号 → 存的 `carrier` 原文。接入时间轴 `TransportTimelineRow.titleView`、详情 `headerSubtitle`/`titleText`、搜索结果卡（优先 `recognized.displayName`）。
