...
- **机场（单一数据源 + 同步零闪）**：把机场不可变数据抽成 `AirportCatalog`（`static let all`/`byIATA`，一次性懒加载），`AirportDatabase` actor 退化为只读 `AirportCatalog.all` 的**搜索引擎**（模糊检索逐键扫表、仍放后台不卡打字），二者共用一份、不重复加载。详情 `TransportDetailView` 经 `.task` + `Task.detached` **后台按需**解析机场名（见下「松解」：撤掉了启动预热，机场库纯按需懒加载，不用航班的用户零开销；代价是详情机场名那行首次一瞬刷新）。机场名只在详情副行露名（时间轴 / Trip Book / 地图本就显码或存值、语言无关）。
...
- **性能 / 改动有效性审计（2026-06-21 松解）**：`Airline`/`Airport` 是不可变值类型，`byIATA` 字典与 `all` 数组共享底层存储（COW，无内容复制）；查名 O(1)。
  - **撤掉「启动预热整个机场库」**：曾为「机场名零闪」在 `CarryApp` 启动后台预热 1.6M 机场库——但这让**不用航班、只用打包的用户每次启动都白白解码 + 常驻几 MB**，只为换详情里机场名（代码下方的次要行）不闪一下。**买卖不划算**（全员买单、少数人受益），按 Minimal Effective Set 撤掉：机场库改**纯按需懒加载**（只在开航班详情/机场搜索时才解码），详情侧 `Task.detached` 把解码放后台、不卡主线程；代价＝首次开某航班详情时机场名那行一瞬「英→中」（次要、可接受）。地图端点注解标题也回退为存值（次要 a11y 标签、且会逼地图一渲染就加载大库）。
  - **航司库（225K）**不预热：随用随载（首个航班行渲染时 ~ms 级解码），高频、用户真看、开销可忽略。
