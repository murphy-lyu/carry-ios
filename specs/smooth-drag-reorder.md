...
> 代码已全部完成并提交在 `feat/smooth-drag-reorder`。下面 ❗ 项**需真机**，全 OK 后告诉我即合并 `main`；✅ 项已用「旧版 build vs 新版 build」在模拟器逐项对比确认一致（真机若顺手可再扫一眼）。

**❗ 需真机确认（找一个长清单，如「西班牙」的 CLOTHING，一并测）**
- [ ] **拖拽手感**：长按行是否快照贴手指 1:1 跟随、其它行平滑让位、不掉帧（核心目标，sim 看不出真实流畅度）。
- [ ] **逐格触感**：拖拽跨行有没有"哒哒"轻触感（起拖有一次较重触感）。
- [ ] **自动滚动 + 跨组夹断**：把某物品拖到屏幕**底/顶边**触发自动滚动 —— 被拖行应**始终夹在原分组内**不串组；同时长 section 内应**还能正常滚到底/顶**够远处的行。
- [ ] **表头吸顶（长列表滚动）**：滚动时分类表头吸顶的观感是否和旧版一致（新旧用同一个 `sectionTitle`，机制 List-sticky → `pinToVisibleBounds`，sim 因清单太短没法验）。
- [ ] **边编辑边拖**：编辑态（键盘弹出）下直接长按另一行 —— 应先提交编辑、本次不启动拖拽（需再长按一次才拖），且不残留空行。

**✅ 模拟器已逐项对比确认与旧版一致（2026-06-08）**
- [x] 静态布局间距 / 位置 / 水平内边距（逐像素）
- [x] swipe 删除外观（红圆 + 白 trash，新旧底层同为 UIContextualAction）
- [x] Dark Mode（黑底/深灰卡/灰分类/白字，逐项相同）
- [x] 大字号 Dynamic Type（XXXL 下行高/换行/各行位置与旧版几乎逐像素一致，无截断）
