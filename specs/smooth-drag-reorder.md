...
> 代码已全部完成并提交在 `feat/smooth-drag-reorder`（7 个提交）。模拟器能测的都过了；下面这些**需要真机**确认，全 OK 后告诉我，我就合并 `main`。

- [ ] **拖拽手感**：长按物品行是否快照贴手指 1:1 跟随、其它行平滑让位、不掉帧（这是本次核心目标，模拟器看不出真实流畅度）。
- [ ] **逐格触感**：拖拽跨行时有没有"哒哒"的轻触感（起拖有一次较重触感）。
- [ ] **自动滚动 + 跨组夹断（最该验的）**：找一个**长清单**（如「西班牙」的 CLOTHING），把某物品拖到屏幕**底边/顶边**触发自动滚动 ——
  - 被拖行应**始终夹在原分组内**，不串到别的分组；
  - 同时长 section 内应**还能正常滚到底/顶**去够远处的行。
- [ ] **边编辑边拖**：点「+ 添加物品」进入编辑态（键盘弹出）后，直接长按另一行 —— 应先提交编辑、本次长按不启动拖拽（需再长按一次才拖），且不残留空行。
- [ ] **swipe 删除**：左滑物品行删除是否正常（红色、无 ghost 动画）。
- [ ] **Dark Mode**：整页在深色下观感正常。
- [ ] **大字号（Dynamic Type）**：放大字号后行高/换行正常。
- [ ] **间距复核**：天气卡到分类、分类间距与改版前一致（模拟器已逐像素对齐，真机再扫一眼）。
