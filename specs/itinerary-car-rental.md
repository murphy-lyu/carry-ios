...
## 增补（2026-06-19）：租车可选字段「车型 / 车牌」

> **Status: 已实现。**

### 动机
租车缺少「你拿到的那台具体的车」的信息。`车型`（Toyota Corolla / 经济型 SUV）帮取车柜台确认、也是旅行记录；`车牌` 在停车场找车、过路费/违章记录时有用。两者是租车版的航班 `座位/机型`——**均非必填、走「有值才显」、空着零噪音**。

### 设计
- `TransportSegment` 加两个带默认值字段 `vehicleModel` / `licensePlate`（空串）——**轻量迁移、单一 SchemaV1 自动处理**（未发布、无线上数据，零升版本）。
- **仅 carRental 暴露**：编辑表单 `moreSection` 在「确认号」后插两行（`mode == .carRental` 才显）；保存时按模式清空（切到其它 mode 不留残值，与机型/座位同款收口）。
- 详情卡排序（字段排序框架，见 design-system）：路线 hero → 确认号 → **车型 → 车牌**（从泛到具体）→ 费用 → 备注。车牌用 `CopyableDetailRow`（可点按复制）。图标：车型 `car.fill`、车牌 `licenseplate`。
- 全链路同步：model + init、备份镜像 `BackupTransportSegment`（可选 + nil 兼容旧备份）+ 读写、`duplicateTrip` 深拷贝、`addTransportSegment` / `updateTransportSegment`。
- 本地化 9 语言：`itinerary.transport.field.vehicle_model`（车型/Vehicle…）、`itinerary.transport.field.plate`（车牌/License plate…）。

## 增补（2026-06-19）：取车/还车详情按端聚焦 + 导航模块

> **Status: 已实现。**

### 动机
租车一条 segment 渲染成取车/还车两事件，但点开**两个事件都进同一张详情卡、航线 hero 把取车+还车两个地址都铺上** → 冗余、且不实用。用户去取车点、去还车点都可能要导航。

### 设计
- `TransportDetailView` 加 `focus: TransportDetailFocus`（`.full` / `.pickup` / `.dropoff`）。`ItinerarySheet.transportDetail` 带上 focus（并入 sheet id，取车/还车是不同身份）；航班/火车等普通交通行 → `.full`（整段双端 hero 不变），租车取车行 → `.pickup`、还车行 → `.dropoff`。
- 聚焦端 hero：单 marker（无连接线）+「取车/还车」小标 + **只显该端地址/时间**。
- 新增**导航卡**：复用 `DirectionsModule`（与地点/住宿同一处逻辑），导航目标 = 该端坐标（取车→`fromCoordinate`、还车→`toCoordinate`）。无坐标 / 无导航 App 不显。`.full`（航班等）不显导航（去"出发站"意义不大，沿用既有取舍）。
- `dayColor` 按聚焦端取（还车用 `arriveDayOrder`），与该事件所在天一致。
- 共享预订信息（确认号/车型/车牌/费用/备注）两端都展示（同一份预订，自洽）。

## 增补（2026-06-19）：交通端点详细地址（取车/还车及火车/巴士/渡轮通用）

> **Status: 已实现。** 出门最实用的就是详细地址，光有地点名不够。

### 根因
`ItineraryPlaceSearchSheet` 回调第 4 参 `address`（`placemark.title`）**本就返回了详细地址，交通编辑却用 `_` 丢弃了**。非反查、非新数据——把这个已到手的地址存下来即可。

### 改动
- `TransportSegment` 加 `fromAddress` / `toAddress`（带默认值、轻量迁移）；地点搜索回调捕获 `address`（不再 `_` 丢弃）。机场搜索（`AirportSearchSheet`）无街道地址 → 航班留空；切到航班 mode 一并清空。还车「同取车」时地址一并拷给还车端。
- 详情 hero：地址显示在**地点名下方**（footnote secondary，最多两行）；与名称/次行/航站楼重复时不重复显示。聚焦取车/还车各显该端地址，整段（火车等）双端各显。
- 全链路同步：model + init、`BackupTransportSegment`（可选 + nil 兼容旧备份）读写、`duplicateTrip`、`addTransportSegment` / `updateTransportSegment`。
- **兼容**：本改动前已保存的交通段无地址（当时未捕获）→ 不显地址；重新选一次地点即回填。无反查、不破坏旧数据。
