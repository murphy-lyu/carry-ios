...
- ~~**总花费 / 分类消费（¥27,622）**：Carry 零记账字段……**坚决不做**。~~
  ...
  >
  > **🔁 二次反转·餐饮/购物分类已纳入（2026-06-21，spec: `itinerary-trip-spend.md`）**：上一条「不做餐饮/购物分类」的依据是**当时**没有分类数据、要做就得新增「让用户填消费类别」的录入。该前提**已不成立**——单行程花费页落地后，`SpendCategory.from(stopCategory:)` 把用户加地点时本来就选的 `StopCategory`（餐厅/景点/活动/购物）**零新增录入**地映射成 7 类消费类别。与上面航班里程/住宿晚数/总花费三次反转**同一逻辑链**（前提变了 → 决策反转）。故 Trip Book 花费卡从「交通/住宿/**地点**」三类升级为按 `SpendCategory` 的细分：交通、住宿，地点拆为**餐饮/景点/活动/购物/其他**（仅显非零类目）。仍克制：① 复用单行程页已有的 `SpendCategory`，跨行程/单行程**同一套口径**不漂移；② Trip Book 维持**单一烟蓝**纪律——不用单行程页的多彩配色，比例带与图例按金额降序走单烟蓝 rank 透明度（额大色深）；③ 不做按天/逐笔/维度切换器（那是单行程操作页的能力，回顾卡不背交互负担）；④ 仍不做分摊/预算/账单导入。`TripSpendStats` 的 `TripSpendBreakdown` 由固定三字段重构为 `[SpendCategory: Double]`。
