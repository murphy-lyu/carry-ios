...
- **航班里程 / 时长 / 机场 Top（CKG 3x）**：Carry 不记录航班号/机场/里程。最多能数 `flight` 类停靠点个数，给不出 Tripsy 的 32,993km、机场排行——做半成品反而劣质，不做。
- **住宿晚数（16 晚）**：无 check-in/check-out 数据。只能数 lodging 停靠点"地点数"，给不出"晚数"。→ 不单列住宿板块（地点数并入 🟡 类别统计即可）。
- **总花费 / 分类消费（¥27,622）**：Carry 零记账字段。要做得新增一整套支出录入，明显超出「打包+轻规划」定位。**坚决不做**——这是 Carry 与 Tripsy 的定位分界线。
