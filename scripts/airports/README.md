...
python3 <repo>/scripts/airports/fetch_cn.py          # 从 Wikidata 抓简繁中文名 → /tmp/cn_names.json
python3 <repo>/scripts/airports/build_airports.py    # 再跑一次，并入中文名
```
...
Wikidata（`P238` = IATA）取 `zh-hans` / `zh-hant` 标签（带 `zh` 兜底）。缺失则显示回落英文原名。
