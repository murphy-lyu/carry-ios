...
>
> **2026-06-17 翻转**：用户反馈空隙穿透到行程卡更恼人 → 在 `bottomActionBar` 加回吸 tap 背景（`.background(Color.clear.contentShape(Rectangle()).onTapGesture { })`，置于按钮之后 → 按钮优先吃 tap、仅空隙 tap 被吸掉）。**代价已知并接受**：底栏那条 ~54pt 区域不再能上滑滚列表（列表照常在自身区域滑动）。tap/pan 两全在此架构下仍做不到，本次明确选「不穿透」。
