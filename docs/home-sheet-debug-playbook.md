...
> ⚠️ **取舍（曾保持现状，2026-06-17 按用户要求翻转）**：去掉基线"透明吸 tap 背景"后，底栏三按钮之间两条约 14pt 空隙的**点击**会穿透到底栏后的列表行（按钮自身 54pt 命中区不穿透）。原因：背景加回会让整条底栏可命中 → pan 也被吞 → 破坏"从底栏上滑滚列表"，二者在"底栏作 UIKit 兄弟视图"架构下互斥（hitTest 无法区分 tap/pan）。
>
> **2026-06-17 翻转**：用户反馈空隙穿透到行程卡更恼人 → 在 `bottomActionBar` 加回吸 tap 背景（`.background(Color.clear.contentShape(Rectangle()).onTapGesture { })`）。
>
> ⚠️ **2026-06-19 已废弃此 SwiftUI 补丁，改为结构性根因解 → 见 §33。** 那个 SwiftUI 吸 tap 背景依赖底栏布局恰好盖住点击区，每次改底栏 UI 就被绕过 → 穿透反复复发（5+ 次）。防穿透已移到 UIKit 根层 `FXPassthroughView.hitTest`（底栏 frame = 不放行区），SwiftUI 层那行补丁已删。**别再加回任何 SwiftUI 吸 tap 层。** 取舍（底栏区不能滚列表）不变。

## 20. 已修复：FX Sheet 展开吸附过冲时底部漏 MapKit（透明卡片 + 固定高内容，2026-06-15，commit `b58b478`）

**现象**：上拉松手展开到顶，克制果冻回弹过冲那一瞬，Sheet 底缘与屏幕底裂出一条缝、漏出后面的地图，落定后缝合。（§19 的果冻回弹上线后才暴露。）

**先走错一步（记录以免重犯）**：第一判断是"卡片底缘被抬起"，去把 `outerView.layer.anchorPoint` 改成底边中心想钉住底边。**逐帧推算证明卡片底缘其实全程 ≥ 屏幕底**（spring 过冲只把底缘往屏幕**外**冲），并未抬起——锚点修的是不存在的问题，无效、已完全回退。**教训：动画类漏底先分清是"卡片位移"还是"内容覆盖不足"，别凭直觉认定是位移。**

**真因（渲染覆盖，非几何位移）**：卡片三层 `outerView/innerView/hostingView` **本身全透明**（代码里都没设 backgroundColor / 是 `.clear`），Sheet 底色仅由 SwiftUI 内容里的 `.background(CarrySubtleBackground())` 画；而 `hostingView` 高度**固定 = expandedHeight、钉在 innerView 顶部**（内容固定尺寸是性能不变量，见 §26/§28）。展开吸附 spring 过冲会把 `innerView.bounds` 瞬间**撑过 expandedHeight** → 底部多出约 56pt 一条「无内容、又无背景」的**透明带** → 漏出 MapKit。

**修复（根因·一行，§5 安全）**：给 `innerView` 自身一层不透明兜底背景 = `CarrySubtleBackground.baseColor`（渐变底端同色，本就为消除底部接缝而设；`ViewModifiers` 暴露 `baseUIColor` 作单一动态色源、不硬编码）。卡片从此不透明：正常态被内容完全盖住、不可见；过冲那条带露出的是与 Sheet 底端无缝同色而非地图。**不碰几何/吸附/手势/内容尺寸**。

> 🔑 **通用教训**：固定高内容 + 透明卡片，遇到「可视窗口可瞬间撑过内容高度」的动画（过冲 / 橡皮筋 / 任何 bounds 超出内容的瞬间）必漏底。**卡片应当有自己的不透明背景，而不是依赖固定高内容去兜全部不透明。** 对照 §7（fallback sheet 当年用 `containerView` 向下延伸 400pt 兜 overshoot——同类问题的另一种解法；FX 改用"卡片自带不透明背景"，更省、更直接）。

## 21. 底部消隐渐变「移到 FX 层、钉卡片可视底边」（2026-06-17）

**问题**：有行程时底部消隐渐变（原 SwiftUI `bottomContentFade`，挂在列表上）**只展开态可见**；收起态 Sheet 被裁短，渐变锚在内容底部（固定 `expandedHeight` 处）→ 被裁到屏外 → 露在收起卡片底边的列表行被硬切、显乱。

**根因**：渐变锚在「内容坐标系的底部」，而收起是 UIKit 层裁剪可视窗口——内容不知道裁剪线，渐变跟不到收起后的可视底边。

**解（FX 新增一层）**：新增 `FXBottomFadeView`（layer 即 `CAGradientLayer`，透明→`CarrySubtleBackground.baseUIColor`），在 `installContent` 里加到 `innerView`（在 hostingView **之后** → z 序在内容之上），`placeSheet` 里把它 `frame` 钉在 `innerView`（= 卡片可视窗口）**底边**、高 120pt（`isListEmpty` 时高 0 不铺）。`innerView` 随收起裁短，渐变自动跟到可视底边 → **展开/收起两态都在可视底部消隐**。SwiftUI 列表上的 `.bottomContentFade` 同步移除（ItemPicker 仍用该修饰器，不受影响）。

**为什么安全（逐条核过）**：① `isUserInteractionEnabled=false` 不拦触摸；② 在 innerView 内、底栏之上的 `view` 层不受影响（渐变不糊底栏）；③ 纯线性渐变 GPU 廉价、`frame` 每帧设是 O(1)、不触发 SwiftUI relayout（保「内容只布局一次」性能不变量）；④ UIView.frame 动画块外无隐式动画（跟手）、snap animator 块内被插值（同步）；⑤ 淡到 `baseUIColor` = innerView 兜底色，§20 过冲带仍同色不漏地图。

## 33. 🔴 底栏点击穿透到行程卡——根因解与「碰这块必守」的防复发规则（2026-06-19）

> **背景：这个问题反复出现了 5+ 次**，每次一调首页底栏（搜索 / Trip Book / 创建）的 UI（间距 / 玻璃 / 背景 / 布局），点击就又穿透到背后行程卡、误开行程。**根因一直没在对的层级修**，每次都在 SwiftUI 层打补丁，下次改 UI 又被绕过。本节是**终结方案**——以后碰这块照这条走，不会再复发。

**架构事实（为什么会穿透）**：首页底栏是 `CarryBottomSheetFX` 里**与行程卡片同级、叠在其上**的 UIKit 兄弟视图（§19 移进控制器换同步缩放）。根视图 `FXPassthroughView.hitTest` 的设计是「没有子视图认领的点就返回 nil、放行给背后（地图 / 卡片）」——这是为了让卡片间隙能透到 MapKit。但底栏是个 `UIHostingController`，**SwiftUI 命中测试只认领有内容 / `contentShape` 的区域**：三按钮之间的空隙、padding、iOS 26 `GlassEffectContainer` 重塑后的边角，这些**透明区 SwiftUI 会放行** → 根视图继续下探 → 撞到背后卡片的 `Button` → 误开行程。

**为什么反复复发（核心教训）**：历次修法都在 **SwiftUI 层**补（如在 `bottomActionBar` 加 `.background(Color.clear.contentShape(Rectangle()).onTapGesture{})` 吸 tap，见旧 §19 脚注）。这类补丁**依赖底栏内部布局恰好盖住点击区**——而底栏 UI 一改（换玻璃容器 / 调间距 / 改背景），透明区域就变了，补丁被绕过，穿透重现。**层级错了：防护放在"会随布局变化的 SwiftUI 内容"里，注定每次改 UI 都重新破。**

**根因解（结构性，放在 UIKit 根层，布局怎么改都盖得住）**：在 `FXPassthroughView.hitTest` 里把**底栏的 frame 设成「不放行区」**——凡落在 `barView` frame 内的点，一律由底栏认领（命中按钮→给按钮；落空隙→返回底栏自身把 tap 吸掉），**永不下传卡片**。与底栏 SwiftUI 内容是否透明**完全无关**，所以底栏 UI 怎么改都不复发。`FXPassthroughView` 持 `weak var barView`（`installBottomBar` 里设），`convert` 用 UIView API 自动处理底栏的缩放 transform。

```swift
// FXPassthroughView.hitTest（节选）
if let bar = barView, bar.window != nil, !bar.isHidden, bar.alpha > 0.01 {
    let pInBar = bar.convert(point, from: self)
    if bar.bounds.contains(pInBar) { return bar.hitTest(pInBar, with: event) ?? bar }
}
let hit = super.hitTest(point, with: event)
return hit === self ? nil : hit
```

**🔒 碰首页底栏（HomeView `bottomActionBar` / 三按钮 / 玻璃容器）必守**：
1. **防穿透的唯一真源 = `FXPassthroughView.hitTest` 的不放行区**。改底栏布局 / 间距 / 背景 / 玻璃**放心改**，根层防护盖全程，无需任何额外处理。
