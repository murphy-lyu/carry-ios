...
> 单会话、无并行。**已提交并 push**（`1704cbb` / `cba0a19`）。

- **真正根因（用户反馈截图与代码逻辑对不上，逐层排查后定位）**：WidgetKit **不会自动把整块画布尺寸提议给 widget 根视图**——内容偏少时，`VStack` 只按内容尺寸收缩，然后被系统在整块画布内居中摆放，末尾的 `Spacer(minLength: 0)` 因为拿不到"画布减去内容"的多余空间可分配、根本不生效。之前几轮反复调整 Spacer/居中逻辑都是在一个已经被系统摆到画布中间的收缩盒子内部瞎折腾，没碰到真正病灶。线索是同文件 `agendaView`（进行中状态用，全程表现正常）早就写了 `.frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .topLeading)`，而 `largePreTripView`/`mediumView`/`smallView`（规划中/即将出发用，全程表现异常）从来没有——现在三处都补齐了同款 `.frame`。
- **排查过程记录**（避免下次重蹈）：中途怀疑过 DerivedData 缓存（清了 4 个旧目录）、怀疑过多项目/多分支（`git worktree list`、全盘 `mdfind` 排查，确认只有一份代码），这些都不是根因，但排查本身是必要的排除步骤。真正定位靠的是对比"全程正常的 agendaView" vs "全程异常的三个函数"两者代码的**唯一差异**。
- **新产品规则（用户反馈）**：规划中 / 即将出发两种 pre-trip 状态，只要行程规划（`WidgetTrip.agenda`，含地点+交通）是空的，统一展示"引导去规划"文案，不展示打包进度；有日期的话倒计时仍展示（跟打包/规划状态无关）。只有行程规划里有 ≥1 项才展示对应内容（规划中→已规划 N 项；有日期→倒计时+打包进度，Medium 的进度环同步收紧到这个条件）。
- 过程中也用本地 HTML 原型（`~/Downloads/carry-widget-mockup.html`）跟用户对齐过一版排版方向（贴顶不居中），最终连同这次根因修复一起生效。
- 主 App / Widget build 通过，无本地化改动。**待办**：真机验收三个尺寸在"规划空/规划中有内容/有日期无规划/有日期有规划"四种组合下的最终效果。

## 上次改动摘要（Widget 布局多轮迭代收敛 · 2026-07-10）

> 单会话、无并行。**已提交并 push**（`21cd271` / `1caa5e6` / `a717e11`）。

- **背景**：用户反馈三个尺寸 Widget 的"规划中/即将出发"状态排版混乱、互不一致。前两轮（`21cd271` `1caa5e6`）分别给 4×4、1×4 的"规划中零内容"状态加了居中 CTA（图标+文案居中放进剩余空间），结果被用户指出这套"居中悬浮"处理反而制造了断层感（标题贴顶、文案孤零零飘在中间），三个尺寸各用一套锚定逻辑（贴底/居中/贴顶）看着更乱。
- **最终方案**（`a717e11`）：推翻居中处理，三个尺寸统一成同一套最简单的规则——header 贴顶，标题/状态行/预览条目紧贴依次排列，不做任何居中或大 Spacer 填空；卡片下方多余空间就是纯留白（对标 Apple 自家日历/提醒事项 Widget 的做法）。去掉 mappin 图标，去掉 isPlanningEmpty 特判分支——净减 43 行，比中间两轮的版本更简单。
- **过程**：先写了本地 HTML 原型（`~/Downloads/carry-widget-mockup.html`，3 尺寸 × 2 相位 × 空/有条目共 8 张卡片）给用户确认排版方向，再落代码，避免继续"改一处发一次截图"的来回。
- **Widget 边距**（`21cd271` `1caa5e6`）：`.contentMarginsDisabled()` + 统一手动 padding 替代 WidgetKit 按 family 给的自动边距；Small 用 12pt、Medium/Large 用 16pt（同一绝对值在窄卡片上占比更大，用户反馈确认过方向）。
- 主 App / Widget build 通过，无本地化改动。**待办**：真机/模拟器验收三个尺寸最终效果。
