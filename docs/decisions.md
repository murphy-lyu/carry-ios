...
## 2026-07-14 反馈/联系邮箱：全项目移除个人 icloud，统一工作室邮箱

**决策**：App 内"意见反馈"与法务页（隐私政策/用户协议/站点页）所有对外联系邮箱，从个人 `murphy.lyu@icloud.com` 统一改为工作室邮箱 `support@nevestudio.app`；此后不再对外暴露个人邮箱。

**范围**：Carry 主仓 2 处（`CoffeeSheetView`/`SettingsView` 的反馈 mailto）+ legal/neve-web 仓 10 处（privacy/terms 中英各 2 + 站点两个 index + README）。

**边界**：`docs/infrastructure.md` 的 `murphy.lyu@hotmail.com` 是 Cloudflare/Azure **账号主体**、非对外联系邮箱，不在此范围、保留。

## 2026-07-13 Widget 地点条目：图标语言全相位统一，地址按相位差异化

**背景**：Widget 三相位（Planning / Upcoming / In-trip）此前地点条目视觉不一致——In-trip 用朴素描边图标、Planning/Upcoming 用类别色圆底；且地址在各处的展示也无统一规则。

**决策一（图标统一）**：三相位同一套**类别色圆底图标**（对标 Tripsy）。同一个"地点条目"组件在全 App 里应是同一种视觉语言，随相位切换两套图标读起来像没对齐的两套设计。属纯执行质量层面，直接统一。配色/图标映射复用既有单一真源（`SpendCategory` / `StopCategoryStyle`），不新造。

**决策二（地址按相位差异化，与图标解耦）**：拆出独立 `showSubtitle` 参数，因两者取舍理由不同——
- **Planning/Upcoming 预览**任务是"浏览我规划了什么"，完整门牌/行政区划是噪音，且长地址被 `lineLimit(1)` 硬切在单词中间观感粗糙 → **不展示地址**（单行）。
- **In-trip** 任务是"我此刻要去哪"，地址是实际导航需要的信息（打车报地址、开地图）→ **展示地址**（两行）。
- 同一条"信息密度服务当下任务"原则，在两个相位得出相反结论——这是**有意的差异**，不是不统一。类比 Apple 地图"导航列表 vs 收藏列表"本就不同密度。

**教训（本轮踩坑）**：无法预览真实渲染尺寸时，别改成明显更高/更占空间的布局再赌它放得下。Small「大图标叠标题上方居中」、Medium「2 条 / 居中卡片」都因垂直更高在小画布溢出被裁，来回撤回数轮。小画布的安全解优先"图标+标题同一行"，居中靠前后 `Spacer` 分配既有留白、不靠加大元素。

**背景**：住宿跨多天时，中间日自动插入「从酒店出发（置首）」和「返回酒店（置末）」两条虚拟行，用途是让用户看到「酒店→第一站」和「最后一站→酒店」的距离参考。

**问题**：若当天一个地点也没有（纯休息日），「返回」会孤零零渲染，没有距离锚点，显得奇怪。旧逻辑「出发」已有 `!timed.isEmpty` 门控，但「返回」无条件渲染。

**决策**：两者合并进同一 `else if !timed.isEmpty` 分支——当天有至少一个地点才同时插出发和返回，否则两者都不渲染。核心判据：出发/返回的唯一价值是提供距离锚点，无地点即无锚点，孤零零渲染只有困惑。

## 2026-06-28 SwiftUI Menu 控件 press 残影：`.clipShape` 施加到 Menu 控件本身

**现象**：点击 `···` 菜单展开后收起，按钮周围出现短暂方形残影（约 1 秒消失）。自然静态状态无此问题。

**排查过程（三次迭代）**：
1. 第一次：以为是 `compositingGroup() + shadow` 把内容拍平成矩形位图再投影，改为 `Shape.fill(.thickMaterial).shadow()`——无效，因 material 底层 UIVisualEffectView CALayer 仍是矩形，shadow 从 bounds 算。
2. 第二次：改为 `.background(.thickMaterial).clipShape().shadow()` 顺序——label 内部渲染正确了，但残影仍在。
3. 第三次：扩大排查范围到 Menu 控件层级。发现真根因：SwiftUI `Menu` 控件的 press/highlight 动画作用在**控件自身的矩形 bounds** 上（不是 label 内容），收起时 highlight 以矩形淡出 → 方形残影。

**根因解**：在 `Menu` 控件（不是 label）上加 `.clipShape(Circle())`，把控件自身的交互层裁成圆形。Menu 弹出层在独立 UIWindow，不受此 clipShape 影响，功能正常。

**教训**：UI 残影类问题，先确认是「渲染内容」还是「控件交互层」——shadow/material 改了三次都没用，因为问题根本不在渲染层，而在控件自身的交互动画层级。

## 2026-06-28 ··· 与编辑按钮颜色：保持主次层级，不统一

**背景**：用户询问 ··· 圆按钮（`ellipsis` 图标 `.secondary` 灰）和编辑胶囊（`CarryAccent` 烟蓝）颜色是否应统一。

**决策**：保持现有层级差异，不统一。理由：
- 编辑是当前页面唯一主操作，accent 色表达「这是主要入口」
- ··· 是溢出菜单入口，内容包含删除等次级/破坏性操作，secondary 灰在视觉上天然「降一档」，符合信息层级
- Apple 日历、地图等自家 app 同款：主操作高亮、溢出菜单入口中性色
- 背景（毛玻璃 thickMaterial）两者已一致，差异只在图标前景色，是**有意义的差异**而非疏漏
...
### Widget `upcomingAgenda`：住宿哨兵 order 必须跳过时间过滤（2026-06-28）
原因：`order = -2`（checkout 哨兵，当天最前）和 `order = Int.max`（checkin 哨兵，当天最末）是住宿特殊标记，不代表实际分钟数。过滤逻辑 `item.order / 2 >= nowMinutes` 若对 `order = -2` 执行，`-2 / 2 = -1 < nowMinutes` 永远为 false，导致当天 checkout 行被错误过滤掉。
决定：时间过滤分支加 `item.order >= 0` 守卫：
```swift
if item.dayOrder == idx, item.order >= 0, item.order < 24 * 60 * 2 {
    return item.order / 2 >= nowMinutes
}
```
哨兵行由下方的全天/住宿专属逻辑分支处理。

### `AgendaRenderRow.moreCount` 永久移除，静默截断（2026-06-28）
原因：「+N more」行曾存在于设计草稿，`moreCount` 字段残留在 struct 里但构造点已删，`agendaView` 里有 `else if let more = row.moreCount {}` 渲染分支但永不被触发——是死代码。
决定：**彻底删除** `moreCount` 字段及渲染分支。超出 `maxSlots` 的条目**静默截断**，不显示「+N more」——Widget 上静默截断比显示计数更简洁，用户知道打开 App 可看全部。**禁止**重新加入该字段或「+N more」行。

### Widget 旅行中相位：跨天夜行段的 `sinceMinutes` 过滤加 `departIsToday` 守卫（2026-06-28）
原因：跨夜航班/夜行火车的 `seg.departDayOrder` 指向「今天」，但 `isToday` 由 `day.sortOrder == fromDayOrder` 得出；当该段被归入行程某一天（其 `dayOrder` 与 `fromDayOrder` 一致），而 `isToday` 为 false 时，`sinceMinutes` 过滤不执行，导致已过去的夜行段仍出现在 Widget。
决定：`widgetAgenda` 的 `sinceMinutes` 过滤增加第二条 isToday 判定：
```swift
...
```
修复在 App 侧（`TripStore.swift`），不在 Widget 侧，与 Widget 侧的 `upcomingAgenda` 动态过滤互补形成两层防护。

### Small Widget 1×1 标题用 `.caption semibold uppercase tracking`，禁用 `.headline`（2026-06-28）
原因：Small Widget 宽度约 155pt，`.headline`（17pt semibold）在英文「Upcoming」上强制换行为「Upcom-\ning」，视觉破碎。
决定：标题统一用 `.caption + .weight(.semibold) + .textCase(.uppercase) + .tracking(0.4)`，视觉上与 Day 分组 section label 保持一致的「标签感」。已写入 design-system.md §桌面小组件。
