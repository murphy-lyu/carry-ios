...
### 按天分色：转暖色基调 + 精简到 7 色 + 绿色打头 + 易读性靠渲染（2026-06-20）
背景：PM 先问扩到 15/31 天，再要求整组转**暖色、愉快、像要出发去玩**，最后定稿为绿色打头、精简到 7 色。
判断（反转伪需求）：**不需要 N 个全局唯一色，只需「局部可区分」**——地图/时间轴上只有彼此挨近的天才需要区分。目标从「凑齐 15/31 个唯一色」改为「**N 色循环 + 任意连续 5 天可分**」，对任意天数都成立。
落地（最终）：
- **7 色、暖色主导**：`Day 1 棕榈绿 → Day 2 万寿菊 → Day 3 烟蓝(品牌) → 珊瑚 → 海蓝绿 → 胭脂粉 → 浆果红`。中途曾扩到 10 色，后**去掉近似色（焦糖/赤陶/豆沙玫）精简到 7**——更干净，CIEDE2000 floor 反升到 **14.4**（10 色时 12.5）。N=7 下任意两色都同处某个 5 天窗（最大循环间隔 3），故 floor=全局最小色差、与顺序无关；顺序只用来**最大化相邻日色差**（相邻最差 ΔE≈16.2）。
- **绿色打头 = 产品决策**：用户要绿色打头（出行心情），**主动放弃「Day 1＝品牌色」的延续性**（2026-06-13 那条），品牌烟蓝移到 Day 3。纯展示、无技术代价。
- **纯暖色被否决（根因）**：暖色只占色相环一小段，全暖时 ΔE 崩到约 8。故暖色主导 + 海蓝绿/棕榈绿点缀（读作海岛、非阴郁冷色）。
- **🔴 易读性靠「改渲染」而非「改颜色」（关键根因）**：暖亮色板里几个浅色（胭脂粉对白底仅 1.97、焦糖/万寿菊/棕榈绿 2.6–2.7）低于 WCAG 3:1，导致**地图针白色序号在浅底上看不清**、浅色路线在浅地图上发虚。**试过加深颜色修对比度——必塌可分性**（浅色承载着亮度分隔维度：降饱和 floor→8.7，整组压暗→6.6，单独压深胭脂粉则并入豆沙玫）。所以根因不在颜色、在「渲染默认日色够深能托白字」这个错误假设。解：① 地图针序号改 `Color.legibleInk`（`AppearanceMode.swift`，按底色 WCAG 亮度自动取深/浅字）；② 浅色路线加暗 casing 衬底；③ 时间轴浅色图标/色点偏柔但有色圈+位置+序号共同标识、可接受、不动。**禁止再用「加深色板」修对比度。**
