...
### 按天分色：精简到 7 色 + 全色环铺开 + 绿色打头 + 易读性靠渲染（2026-06-20）
...
落地（最终）：
- **7 色、色相绕色环铺开**：`Day 1 棕榈绿 → Day 2 万寿菊 → Day 3 烟蓝(品牌) → 雪青 → 覆盆子 → 青绿 → 赤陶`（绿/橙/蓝/紫/莓红/青/赭红）。每天一个清楚不同的「颜色名」。CIEDE2000 floor **14.7**，相邻最差 ΔE≈16（蓝↔紫，色环邻居、灰蓝 vs 饱和紫仍可分）。前四天（绿/橙/蓝/紫）按用户偏好钉死，后三天求解、并避免覆盆子与赤陶相邻。
