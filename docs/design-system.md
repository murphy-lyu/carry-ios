...
> ⚠️ **按天分色 = 单一强调色原则的唯一破例（2026-06-13 定，仅限行程规划）**：见 `decisions.md`。`ItineraryDayPalette`（`AppearanceMode.swift`）按 `ItineraryDay.sortOrder` 取色：**7 色循环**（`sortOrder % 7`）、明暗自适应。顺序：**Day 1 烟蓝（品牌 CarryAccent）→ Day 2 万寿菊 → Day 3 棕榈绿 → 雪青 → 覆盆子 → 青绿 → 赤陶**——色相绕色环铺开（蓝/橙/绿/紫/莓红/青/赭红），每天是一个清楚不同的「颜色名」。Day 1 用回品牌烟蓝（品牌延续性），绿色移到 Day 3。**只**用于行程的「数据节点」（地图针/路线、时间轴序号圆点/连线/类别图标、Day 头部色点）；行程内的**控件**（按钮 tint）仍用 `CarryAccent`。`ItineraryDayPalette` 禁止在行程规划之外引用。
>
> **配色约束（根因）**：曾试「暖色主导」版，但暖色只占色相环一小段，挤进 7 个会出现「三个粉红一家」的近似感（用户实测嫌弃）。定稿改为**全色环铺开**：CIEDE2000（明暗两套取较差值）验证 **7 色两两 ≥ ΔE 14.7**，排序再最大化相邻日色差（相邻最差 ΔE≈20）。N=7 下任意两色都会同处某个 5 天窗（最大循环间隔 3 ≤ 4），故 floor = 全局最小色差、与顺序无关——7 色必须个个分得开。前四天（蓝/橙/绿/紫）按用户偏好钉死，后三天求解。
>
> **易读性（关键）**：这套色偏亮/愉快，部分浅色在白底上低于 WCAG 3:1。**根因不在颜色、在渲染假设**——「日色够深能托白字」这个假设错了。故**画在日色上的前景元素禁止写死白色**：地图针序号用 `Color.legibleInk`（按底色明暗自动取深/浅字）；浅色路线加暗 casing 衬底。**禁止靠加深色板来「修对比度」**——浅色承载着可分性，压暗会让 ΔE 崩塌（已实测：降饱和 floor 掉到 8.7、整组压暗掉到 6.6）。**改色前必须重跑 `scripts/itinerary-day-palette-solve.py` 复核**。颜色由 `sortOrder` 派生、纯展示层，不进 model/备份/schema，零迁移。
...
### Trip Book「总花费」卡（数据可视化的克制范式 · 2026-06-21 升级为 7 类）
对标 Tripsy「总花费」卡做了 Carry 化（north-star §1/§2/§4）：
- **比例带 = 把「一列数字」变成「一眼看懂的构成」**：总额下方一条 8pt capsule，按 `SpendCategory` **7 类**（交通/住宿 + 地点拆 餐饮/景点/活动/购物/其他，仅显非零）占比分段、按金额降序。这是内容（这屏要回答「钱花哪了」）、不是装饰；对标 Apple Health / Screen Time。
- **单一色相、rank 深浅**：比例带与图例点用烟蓝 `CarryAccent`，**按金额降序走 rank 透明度**（额大 1.0 → 额小 0.28）——守「彩色=意义、不堆色」的单一强调色纪律，深浅本身编码大小。**禁止**给类目分配不同色相。注：单行程花费页（`TripSpendView`）才用多彩类目色（操作型详情）；Trip Book 是克制回顾，刻意保持单烟蓝。
- **「最高一趟」texture 行**：图例下一行（≥2 趟才出）点出花费最高的那趟（中性措辞，不渲染超支），把「按行程」维度抬到卡面、并作「View all」引子。
...
### Trip Book「查看全部花费」= 时间轴流水账（2026-06-21）
- 按**时间倒序**：年=分段大标题（带**年度小计**）→ 每趟左侧**日期标记**（月日合一、烟蓝）→ 趟内逐行明细。**整宽卡片**（日期不占左列，避免右偏）。无日期行程归底部「未排期」组。
- **交通按方式拆**：下钻里交通段按 `TransportMode` 分行（航班/火车/租车…各自图标，单烟蓝），不塌缩成一个「交通」行（一行只能挂一个图标 → 永远错）。各方式 + 其余类目相加 = Trip total。
- 字面「年→月→日→行程」四级**压成三级**（每趟仅一日期，拆月/日两级=空壳标题）。

### 列表卡统一规则：「预览 N + View all」（2026-06-21）
适用 Trip Book 的国家 / 机场 / 大洲等「计数列表卡」：
- **自然有界的列表全展示**（大洲 ≤7）；**无界列表预览 10 + 「View all」全列表 sheet**（国家/机场）。**禁止静默截断**（旧机场「+N 个」兜底已废）。
- affordance 走 Apple「See All → 目标页用内容名」：**按钮一律泛化「View all」、全列表 sheet 标题用各自卡片名**（不自我重复，如花费 sheet 标题=「总花费」而非「查看全部花费」）。
...
- **行高两档（2026-06-28 定稿）**：
  - **定时日历事件** = `TimelineRail.rowHeight`（52pt），与规划行一致——它插入时间轴与行程行并排，垂直节奏必须统一；「退后」靠浅字 + 细竖条 + 无图标承担，不靠行高变矮。
  - **全天日历事件** = **30pt**（紧凑信息条）——全天事件是纯背景标注（节假日/纪念日），无时刻锚点、不占时间轴位置，用 52pt 与行程行等高会过度占用视觉重量。30pt = footnote 字高 + 8pt 上下余量，退到「信息条」层级，与行程行视觉层级明确区分。
- **排列（2026-06-23 定稿）：全天事件钉当天顶部，定时事件按时序插入主脊。** 全天事件无时刻 → 留顶部「当天背景信息」band；定时事件（有起始时刻）按 `startMinutes` 落到主脊它的时间位（与停靠点/交通/住宿同一条按时序的脊），因为**时间轴的核心承诺就是按时序读**——把 18:00 的日历事件钉在 10:35 航班之上等于让时间轴撒谎、也看不出冲突。实现：`timelineRowIDs(timedCalendar:)` 按分钟插入；全天经 `daySections` 留顶部。
- **脊在日历事件处自然断开（不连续）**：定时事件落在脊的时间位，但**主脊的灰线在它处断开**——相邻规划行不朝它画连线（`showsTopLine/showsBottomLine` 邻接判定遇日历行即清线），它仍只用「日历色细竖条」标识、不接规划脊。语义：它是「来自你日历的外部叠加项」、不属于你规划的路线,断开诚实表达这一点。竖条落在 rail 列、与上下图标同心,所以断开读成「这一行换了个来源标记」而非「线坏了」。
  - **否决过的「脊连续穿过」方案（留依据）**：试过让脊连续穿过日历事件 + 给它一个 rail marker。① 空心环 = 太抢眼、比规划节点(淡灰实心圆)还跳 → **层级倒置**（退后的叠加项成了最响的 marker）；② 改小实心点后虽退后、脊也连续，但**最终选 A（断脊 + 竖条）**：外部叠加项与规划路线「明确分离」比「脊全连续」更重要，断开本身就是正确的语义信号。

### 时间轴「日期头」分节层级（2026-06-22 定稿）
逐日时间轴里每天的日期头（`Sat, May 2`）要明显是这一组的**父级**、能让用户一眼把列表「切块成天」。层级**纯靠字号/字重 + 留白节奏 + 当天色点**建立，不靠线/盒/需要语境解读的符号：
- **标题 `.title3` rounded bold（20pt）**——明显高于行标题（`.body` rounded **semibold 17pt**）一档且更重。**根因**：二者原本同号同重（都 17），只靠一个 8pt 点区分 → 读成等重平流。拉开字号字重是去掉「平」的根。
- **当天色圆点 10pt**（`ItineraryDayPalette` 当天色），与 20pt 大标题光学配重；落在 rail 标记列（30 宽）压在连续脊上。仍是「点」非「盘」。
- **每天上方留白 `.padding(.top, 22)`**——节奏上把「新的一天」断开。
- **否决过的两个方案（留依据、勿重复纠结）**：① **Tripsy 式上下细线**——Tripsy 事件是独立卡片、日期头是卡片组间的裸文字带，可用横线;但 Carry 是**连续竖直 rail**，横线会**十字横切竖脊**，架构不匹配（且违 north-star §1 退后）。② **rail 上放「当天色圆盘 + 日数字」**（呼应顶部日期条选中胶囊）——脱离日期条单独看时，**圆里的数字失去「这是几号」的语境、用户读不懂**（胶囊的可读性依赖其上方「Sat」注解，搬到时间轴就断了）。结论：日期头突出**只用排版**，不引入需语境解读的新符号。
...
## 行程详情 / 编辑：统一设计语言（2026-06-22 定稿 · spec 串：itinerary 详情/编辑/创建 视觉收口）

> 一句话总纲：**「编辑 = 把详情变得可改」**。详情与编辑共用同一画布、同一卡片骨架、同一字段排序框架（见上「活动详情卡：字段排序框架」）；编辑态只是把只读行换成输入控件，**绝不另起一套视觉**。统一是「处理方式一致」，不是「强行同槽位」——美的前提下统一，绝不以统一牺牲美（见 [[carry-unify-in-service-of-beauty]]）。适用全部编辑/创建页：地点、住宿、航班/交通、租车。

### 画布与骨架
- 详情与编辑都用 `carryCanvas`（`systemGroupedBackground`）+ `.carryCard` 卡片；编辑页用原生 `Form`，但 Section 的分组/留白要读成「详情卡的可编辑版」。
- 字段排序**两端一致**，都走五档框架（骨架 → 定位/凭据 → 描述规格 → 费用 → 备注 → 附件）。**费用 / 备注 / 附件**在详情是三张独立卡、在编辑是三个独立 Section，固定顺序，不与类型字段混排。

### marker / 图标语义：详情语义化，编辑通用化
- **详情**用**语义化 marker**——航线 hero 卡按 mode 取最贴切符号（飞机/火车/巴士/渡轮 = 方向箭头 `↗`/`↘`；租车 = 钥匙 `key.fill`），地点/住宿用对应语义点。这些是「读懂这条安排」的信息载体。
- **编辑**里所有「定位 / 搜索回填」的端点统一用**通用 `mappin`**（地址行 marker、定位按钮图标都是 mappin）。编辑态的语义是「在哪儿、去哪搜」，钥匙/箭头/床这类语义符号在编辑里会显得**刻意、排版乱**（曾在租车取/还车端强加钥匙，反而乱）→ 一律退化为 mappin。**语义符号只活在详情。**

### 图标要「挣来」(earned icons)，不平均散发
- **密集的 More 表**（确认号、车牌、航站楼、座位…这类一行一字段的清单）**一律不带 leading icon**——图标在这里只会制造视觉噪声、抢注意力。
- **单一用途的功能卡 / 行**（费用 `creditcard`、备注 `note.text`、链接、电话 `phone`/`CallableDetailRow`）**可带 leading icon**——它是这张卡的「身份」，挣得起。
- **端点行**（出发/到达/取还车/入住退房定位）带 marker——它承载方向/定位语义。
- 判据：图标是否在传达「这一行是什么类别 / 什么语义」？是 → 留；只是「装饰性地让每行都有个图标」→ 删。

### Section 头 = 实体名
- 编辑页每个分组的 Section header 用**实体名**而非泛词：地点 `itinerary.category.lodging` 之于住宿、`Place` / `Lodging` / `Flight` / `Car rental` 经 `mode.localizationKey` 取。航班、租车顶部也加实体名 header，与地点/住宿对齐 → 四类编辑页头部结构统一。
- 「More」类杂项分组（Booking code、确认号等）统一用 `itinerary.transport.section.more` 作 header。

### 时间的两种规范呈现
- **详情**：时间走**右对齐独立列**（`LabeledDetailRow.trailing`，monospaced），与左侧标签拉开成表格感，不用「· 拼接」那种粗糙串接。
- **编辑**：时间走**等宽 chip**（`FormChip` + `monospacedDigits`），日期/时间各一枚 chip；不是 toggle+内联（避免行高跳变，见 [[carry-form-datetime-chip-pattern]]）。
- **日程只展示一次**：模块内已有日期/时间，就**不在标题栏重复**（住宿曾「标题栏日期 + 模块日期」三行重复 → 砍掉标题栏那行）。

### 空字段用「示例占位」(example placeholder)
- 编辑页空字段的 placeholder 给**真实示例**而非泛指令——确认号 `ABC123`、车型 `Porsche 911 Turbo S`（已本地化）、车牌/航站楼/座位/机型各给中性示例 pattern。让用户一眼懂「这里该填成什么样」。占位文案全部进 xcstrings（`*.placeholder` key），9 语同步。

### 删除只在详情的「···」，编辑页无删除
- **编辑页底部不放删除**。根因：详情→点编辑→在编辑弹层里删除→编辑层关闭→回落到详情弹层，但该实体已被删 → **衔接不上、展示已删对象**。删除统一收到**详情弹层右下「···」菜单**（删完直接 dismiss、干净）。航班同此规则。
- 详情「···」里的删除**去掉二次确认**——删一个地点/段无重大损失、也不存在误点场景，二次确认是多余摩擦。

### 详情复制：两种形式（值=长按即复制，标题=系统 Copy 菜单）（2026-06-23）
详情卡的复制分两种,各司其职:
- **值字段 = 长按即复制 + 居中「已复制」Toast**(`longPressCopy` 修饰器:`UIPasteboard` + 轻震 + Toast + VoiceOver「拷贝」自定义动作)。**手势统一为长按**(不是轻点)——轻点留给字段本职(电话拨号 / 链接打开 / 地址将来导航),长按统一=复制,避免滚动误触。**该复制的字段**(按「会粘到别处/出示/发人」判据):地址(地点/住宿/租车取还车/日历事件)、确认号·预订码(航班/租车/住宿)、车牌(租车)、电话(轻点拨号 + 长按复制)、URL(轻点打开 + 长按复制)。**不复制**:时间/日期/晚数/舱位/车型/机型/距离/费用/航司·公司名/座位/航站楼(只读、不外粘)。
- **标题栏 = 长按出系统原生「拷贝」菜单**(`DetailSheetHeader` 标题+副标题加 `.textSelection(.enabled)`)。承载标题里的身份字段(航班号 / 酒店名 / 地点名 / 活动名 / 租车公司)的复制,不必在下方再单列复制行。
- Toast 文案复用通用「已复制 / Copied」(`itinerary.stop.detail.address_copied` 其值本就是通用复制成功);VoiceOver 动作名 `itinerary.detail.copy_action`(「拷贝」)。

### 搜索优先的添加流（search-first add）
- 添加地点/住宿：**先弹搜索 sheet**（搜即所得 + 结果列表），底部固定一个「搜不到？手动添加」入口（`NavigationStack` push 手动表单）。不要一上来叠两层 sheet。
- 选中搜索结果后**不直接落库**，而是**带着回填数据 push 到添加/编辑表单**，让用户补日期等关键字段再确认（住宿尤其要选入住/退房，否则默认只有 1 晚）。
- 搜索 sheet 的标题用**动作名**（`Add lodging`），不要误用字段 placeholder（曾显示「Hotel / place」）。
