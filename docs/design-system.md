...
4. **费用** — 财务信息，规划/记账时看，**现场执行时不是首要 → 压在体验性信息之后**，禁止上浮到骨架/定位之前。
5. **备注** — 自由补充，永远垫底。

落地顺序（`infoCard` / `infoRowViews` 里硬编码、"有值才显"）：
- **交通**（`TransportDetailView`）：**航线 hero 卡**（出发/到达单拎一卡）→ 信息卡（座位 → 确认号 →〔租车：车型 → 车牌〕→ 时长 → 机型 → 距离 → 费用 → 备注）
- **地点**（`StopDetailView`）：时间 → 地址 → 费用 → 备注
- **住宿**（`LodgingDetailView`）：入住 → 退房 → 晚数 → 地址 → 确认号 → 费用 → 备注
