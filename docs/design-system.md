...
> ⚠️ **按天分色 = 单一强调色原则的唯一破例（2026-06-13 定，仅限行程规划）**：见 `decisions.md`。`ItineraryDayPalette`（`AppearanceMode.swift`）按 `ItineraryDay.sortOrder` 取色：**7 色循环**（`sortOrder % 7`）、明暗自适应。顺序：**Day 1 棕榈绿 → Day 2 万寿菊 → Day 3 烟蓝（品牌 CarryAccent）→ 雪青 → 覆盆子 → 青绿 → 赤陶**——色相绕色环铺开（绿/橙/蓝/紫/莓红/青/赭红），每天是一个清楚不同的「颜色名」。绿色打头是产品决策（出行心情），品牌延续性不再绑 Day 1（烟蓝移到 Day 3）。**只**用于行程的「数据节点」（地图针/路线、时间轴序号圆点/连线/类别图标、Day 头部色点）；行程内的**控件**（按钮 tint）仍用 `CarryAccent`。`ItineraryDayPalette` 禁止在行程规划之外引用。
>
> **配色约束（根因）**：曾试「暖色主导」版，但暖色只占色相环一小段，挤进 7 个会出现「三个粉红一家」的近似感（用户实测嫌弃）。定稿改为**全色环铺开**：CIEDE2000（明暗两套取较差值）验证 **7 色两两 ≥ ΔE 14.7**，排序再最大化相邻日色差（相邻最差 ΔE≈16，蓝↔紫那处，蓝紫是色环邻居、灰蓝 vs 饱和紫仍可分）。N=7 下任意两色都会同处某个 5 天窗（最大循环间隔 3 ≤ 4），故 floor = 全局最小色差、与顺序无关——7 色必须个个分得开。前四天（绿/橙/蓝/紫）按用户偏好钉死，后三天求解。
>
