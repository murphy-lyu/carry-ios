//
//  CarryBottomSheetFallback.swift
//  Carry
//
...
//
//  View hierarchy inside FallbackSheetViewController.view (full-screen):
//
...
struct CarryBottomSheetFallback<Content: View>: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> FallbackSheetViewController {
        let vc = FallbackSheetViewController(
    ...
    func updateUIViewController(_ vc: FallbackSheetViewController, context: Context) {
        context.coordinator.mapCityOpacityBinding = $mapCityOpacity
    ...
    final class Coordinator {
        weak var sheetVC: FallbackSheetViewController?
        var hostingVC: UIHostingController<AnyView>?
        var mapCityOpacityBinding: Binding<Double>?
    }
}

// MARK: - FallbackSheetViewController

final class FallbackSheetViewController: UIViewController {
    ...
    private var delegateProxy: FallbackDecelerationCanceller?
    ...
    private func installProxy(on sv: UIScrollView) {
        let proxy = delegateProxy ?? FallbackDecelerationCanceller()
...
extension FallbackSheetViewController: UIGestureRecognizerDelegate {
...
// MARK: - FallbackPassthroughView

private final class FallbackPassthroughView: UIView {
...
// MARK: - FallbackDecelerationCanceller

private final class FallbackDecelerationCanceller: NSObject, UIScrollViewDelegate {
