...
// MARK: - Country centroids (file-level so it's actor-independent)

/// Geographic centre-points keyed by ISO 3166-1 alpha-2 country code.
/// Kept at file scope so `nonisolated` geocode helpers can access it freely.
private func coordinatesForCountry(_ code: String) -> (lat: Double, lon: Double)? {
    tripStoreCentroids[code.uppercased()]
}

private let tripStoreCentroids: [String: (lat: Double, lon: Double)] = [
