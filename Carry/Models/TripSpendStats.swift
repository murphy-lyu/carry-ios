...
//  Carry
//
//  Trip Book 花费聚合（spec: itinerary-cost-tracking.md）。
//  把已发生行程下所有费用按本位币聚合，分「交通 / 住宿 / 地点」三类目；优先用每笔快照、
//  缺失退实时折算、再缺 → 计入「未折算」并诚实标注，绝不静默漏算。
//
...
struct TripSpendBreakdown: Equatable {
    ...
    var lodging: Double = 0
    var places: Double = 0
    /// 是否记录过费用（即便某笔因无汇率未计入合计，也为 true）。
    var hadAnyRecorded: Bool = false

    var total: Double { transport + lodging + places }
}
...
struct TripSpendStats {
    ...
    static func compute(trips: [TripBundle], homeCode: String,
        ...
        for trip in trips where trip.countsAsVisited {
            ...
            func add(_ entity: CostBearing, _ keyPath: WritableKeyPath<TripSpendBreakdown, Double>) {
            ...
            for day in trip.safeItineraryDays {
                for stop in (day.stops ?? []) where stop.hasCost { add(stop, \.places) }
            ...
            if b.hadAnyRecorded {
                stats.overall.transport += b.transport
                stats.overall.lodging += b.lodging
                stats.overall.places += b.places
