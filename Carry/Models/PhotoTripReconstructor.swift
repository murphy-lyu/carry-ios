...
nonisolated enum PhotoTripReconstructor {
    ...
    /// 读所选照片（慢：逐张载入数据、解 EXIF、生成缩略图）。串行处理 + 即时释放，内存有界。
    /// 每张 → PhotoDraft（缩略图 + 归一坐标 / nil）。不请求任何相册权限。
    /// `onItem(已处理数, 本张缩略图)` 每张后回调——驱动「真实缩略图逐张浮现」的加载态。
    /// 支持 Task 取消（用户中途退出即止）。
    static func extract(
        ...
        drafts.reserveCapacity(items.count)
        for (idx, item) in items.enumerated() {
            if Task.isCancelled { break }
            // loadTransferable 拿到所选图原始字节（HEIC/JPEG 等）；本轮处理完即释放。
            var thumbnail: Data?
            if let data = try? await item.loadTransferable(type: Data.self) {
                let meta = exifMetadata(from: data, isChinaStorefront: isChinaStorefront)
                thumbnail = meta.thumbnail
                drafts.append(PhotoDraft(
            ...
            await onItem(idx + 1, thumbnail)
        }
        return drafts
    }

    /// 组装（快：聚类 + 命名）。从已读取的 PhotoDraft 重建草稿。
    static func assemble(
        ...
        // 反向编码命名。地点不多，顺序限流（~1 req/s）即可。
        let geocoder = CLGeocoder()
        var geocodedCount = 0
        var days: [DayDraft] = []
        for dc in dayClusters {
            var places: [PlaceDraft] = []
            for cluster in dc.places {
                if geocodedCount > 0 { try? await Task.sleep(for: .milliseconds(400)) }
                geocodedCount += 1
                let (name, address) = await resolvePlaceName(geocoder, at: cluster.centroid)
                let photos = cluster.photoIndices.map { valid[$0] }
                places.append(PlaceDraft(
                    name: name,
                    address: address,
    ...
    private static func parseExifDate(_ s: String) -> Date? { exifDateFormatter.date(from: s) }
